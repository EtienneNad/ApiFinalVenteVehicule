<?php

namespace App\Domain\Vehicule\Repository\Utilisateur;

use PDO;
 /// Repository.
class AfficherUtilisateurRepository
{
    /**
     * @var PDO The database connection
     */
    private $connection;
    
     /// Constructor.
    
     /**
     * @param PDO $connection The database connection
     */
    public function __construct(PDO $connection)
    {
        $this->connection = $connection;
    }
    public function SelectToutUtilisateur(): array
    {
        $sql = "SELECT * FROM utilisateurs";

        $query = $this->connection->prepare($sql);
        $query->execute();

        $resultat = $query->fetchAll(PDO::FETCH_ASSOC);

        return $resultat;
    }
    public function SelectUtilisateurId(int $utilisateurId): array
    {
        $params = [
            "id" => $utilisateurId,
        ];
        $sql = "SELECT * FROM utilisateurs WHERE id = :id";

        $query = $this->connection->prepare($sql);
        $query->execute($params);
        if ($query){
            $resultat = $query->fetchAll(PDO::FETCH_ASSOC);
        }
        else{
            return [];
        }

        return $resultat;
    }

    public function SelectUtilisateurMotDePasse(string $username): string
{
    $params = [
        "username" => $username,
    ];
    $sql = "SELECT motdepasse FROM utilisateurs WHERE username = :username ";

    $query = $this->connection->prepare($sql);
    $query->execute($params);
    
    $resultat = $query->fetchColumn(); // Utiliser fetchColumn() pour obtenir directement la valeur de la colonne
    
    if ($resultat === false) { // Vérifier si la requête a échoué
        return ''; // Retourner une valeur par défaut appropriée
    }
    
    return $resultat;
}


    public function SelectUtilisateurCle(string $username, string $motdepasse): array
    { 
        $motdepasseHash = $this->SelectUtilisateurMotDePasse($username);
        $params = [
            "username" => $username,
            "motdepasse" => password_verify($motdepasse, $motdepasseHash)
        ];
        $sql = "SELECT cle FROM utilisateurs WHERE username =:username AND motdepasse =:motdepasse";

        $query = $this->connection->prepare($sql);
        $query->execute($params);
        if ($query){
            $resultat = $query->fetchAll(PDO::FETCH_ASSOC);
        }
        else{
            return [];
        }

        return $resultat;
    }
   
}
?>
